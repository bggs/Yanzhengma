...
public class AuthCodeView extends View {
    ...
    public static final int POINT_NUM = 100;   // 点数设置
    public static final int LINE_NUM = 2;// 线段数设置
    ...
    public AuthCodeView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.AuthCodeView, defStyle, 0);//获得我们所定义的自定义样式属性
        //获取在attr文件下，名字为AuthCodeView的declare-styleable属性有几个
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.AuthCodeView_titleText://这个属性可以不要，因为都是随机产生
    ...
    protected void onDraw(Canvas canvas) {
        mPaint.setColor(Color.BLUE);//画背景颜色
        canvas.drawRect(0, 0, getMeasuredWidth(), getMeasuredHeight(), mPaint);
        mPaint.setColor(mTitleTextColor);//划线
        int[] line;
